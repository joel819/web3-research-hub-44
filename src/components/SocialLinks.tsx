...
interface SocialLinksProps {
  data: { twitter: string; linkedin: string; reddit: string; github: string };
  isEditing: boolean;
...
const SocialLink = ({ href, icon }: { href: string; icon: React.ReactNode }) => (
  <a
    ...
    className="text-muted-foreground hover:text-primary transition-colors duration-200 p-2.5 rounded-full hover:bg-primary/10"
  >
    {icon}
  </a>
);

const SocialLinks = ({ data, isEditing, onUpdate }: SocialLinksProps) => {
  if (isEditing) {
    return (
